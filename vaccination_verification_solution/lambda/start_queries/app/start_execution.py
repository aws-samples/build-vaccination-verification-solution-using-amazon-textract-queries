...
def lambda_handler(event, _):
    ...
    for record in event['Records']:
        ...
        if s3_bucket and s3_key:
            ...
            queries_config = [
                tm.Query(text="What is the Name?", alias="CERT_BENF_NAME", pages=["*"]),
                tm.Query(text="What is Vaccination Status?",
                         alias="CERT_VAC_STATUS",
                         pages=["*"]),
                tm.Query(text="What is Date of Birth?", alias="CERT_DOB", pages=["*"]),
                tm.Query(text="What is Document Number?", alias="CERT_PASS_NO", pages=["*"])
